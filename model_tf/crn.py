...
class CRN(Model):
    """
    完整 CRN 模型 (帶 skip connections)
    
    Input: [B, F, T, 1] (channels_last)
    Output: [B, F, T, 1]
    """
    def __init__(self, name="CRN"):
        ...
        self.enc2 = CausalConvBlock(32, name='enc2')
        self.enc3 = CausalConvBlock(64, name='enc3')
        self.enc4 = CausalConvBlock(128, name='enc4')
        self.enc5 = CausalConvBlock(256, name='enc5')
        ...
        # Decoder (帶 skip connections，所以 input channels 加倍)
        self.dec1 = CausalTransConvBlock(128, name='dec1')  # 256+256 -> 128
        self.dec2 = CausalTransConvBlock(64, name='dec2')   # 128+128 -> 64
        self.dec3 = CausalTransConvBlock(32, name='dec3')   # 64+64 -> 32
        self.dec4 = CausalTransConvBlock(16, output_padding=(1, 0), name='dec4')
        self.dec5 = CausalTransConvBlock(1, is_last=True, name='dec5')
    ...
    def call(self, x, training=None):
        ...
        e5 = self.enc5(e4, training=training)  # [B, F', T, 256]
        
        # Reshape for LSTM: [B, F', T, C] -> [B, T, F'*C]
        batch_size = tf.shape(e5)[0]
        freq_bins = e5.shape[1]  # 靜態 shape
        time_steps = tf.shape(e5)[2]
        channels = e5.shape[3]
        
        lstm_in = tf.transpose(e5, [0, 2, 1, 3])  # [B, T, F', C]
        lstm_in = tf.reshape(lstm_in, [batch_size, time_steps, freq_bins * channels])
        
        # LSTM
        lstm_out = self.lstm(lstm_in, training=training)  # [B, T, 1024]
        
        # Reshape back: [B, T, 1024] -> [B, F', T, 256]
        lstm_out = tf.reshape(lstm_out, [batch_size, time_steps, freq_bins, channels])
        lstm_out = tf.transpose(lstm_out, [0, 2, 1, 3])  # [B, F', T, 256]
        
        # Decoder with skip connections
        d1 = self.dec1(tf.concat([lstm_out, e5], axis=-1), training=training)
        d2 = self.dec2(tf.concat([d1, e4], axis=-1), training=training)
        d3 = self.dec3(tf.concat([d2, e3], axis=-1), training=training)
        d4 = self.dec4(tf.concat([d3, e2], axis=-1), training=training)
        d5 = self.dec5(tf.concat([d4, e1], axis=-1), training=training)
...
class MiniCRN(Model):
    ...
    Output: [B, F, T, 1]
    """
    def __init__(self, n_fft=100, name="MiniCRN"):
        super().__init__(name=name)
        self.n_fft = n_fft
        ...
        # Encoder
        self.conv1 = layers.Conv2D(16, (3, 2), strides=(2, 1), padding='same')
        self.conv2 = layers.Conv2D(32, (3, 2), strides=(2, 1), padding='same')
        self.conv3 = layers.Conv2D(64, (3, 2), strides=(2, 1), padding='same')
        
        # 使用 LayerNormalization 代替 GroupNorm(1, ...)
        ...
        # LSTM - 會在 build 時根據實際維度建立
        self.lstm = None
        
        # Decoder
        self.deconv1 = layers.Conv2DTranspose(64, (3, 2), strides=(2, 1), output_padding=(1, 0))
        self.deconv2 = layers.Conv2DTranspose(32, (3, 2), strides=(2, 1), output_padding=(1, 0))
        self.deconv3 = layers.Conv2DTranspose(16, (3, 2), strides=(2, 1), output_padding=(1, 0))
        self.deconv4 = layers.Conv2DTranspose(1, (3, 2), strides=(2, 1), output_padding=(1, 0))
        
    def build(self, input_shape):
        # 計算 encoder 輸出的 feature dimension
        # input_shape: [B, F, T, 1]
        # 經過 3 層 stride=2 的 conv: F -> F/2 -> F/4 -> F/8
        f = input_shape[1]
        for _ in range(3):
            f = (f + 1) // 2
        
        # LSTM input_size = channels * freq_bins = 64 * f
        feat_dim = 64 * f
        self.lstm = layers.LSTM(
    ...
    def call(self, x, training=None):
        ...
        e3 = self.act(self.norm3(self.conv3(e2)))  # [B, F', T, 64]
        ...
        # LSTM
        lstm_out = self.lstm(lstm_in, training=training)  # [B, T, hidden_size]
        ...
        out = tf.nn.relu(self.deconv4(d3))
        ...
        if f_out is not None:
            if f_out > self.freq_bins:
                ...
                out = tf.pad(out, [[0, 0], [0, pad_amt], [0, 0], [0, 0]])
        
        return out


class MiniCRN_Causal(Model):
    """
    因果版 MiniCRN
    ...
    Output: [B, F, T, 1]
    """
    def __init__(self, n_fft=100, name="MiniCRN_Causal"):
        super().__init__(name=name)
        self.freq_bins = n_fft // 2 + 1
        self.hidden_size = 64
        ...
        # Decoder (Causal)
        self.dec1 = CausalTransConvBlock(32, name='dec1')
        self.dec2 = CausalTransConvBlock(16, name='dec2')
    ...
    def call(self, x, training=None):
        ...
        e1 = self.enc1(x, training=training)
        e2 = self.enc2(e1, training=training)
        e3 = self.enc3(e2, training=training)  # [B, F', T, 64]
        ...
        # LSTM
        lstm_out = self.lstm(lstm_in, training=training)
...
class MiniCRN_Causal128(Model):
    ...
    def call(self, x, lstm_states=None, training=None):
        ...
        # 處理 NaN (如果有的話)
        lstm_out = tf.where(
            tf.math.is_nan(lstm_out),
            tf.zeros_like(lstm_out),
            lstm_out
        )
        ...
        out = tf.cond(
            f_out > self.freq_bins,
            lambda: out[:, :self.freq_bins, :, :],
            lambda: tf.cond(
                f_out < self.freq_bins,
                lambda: tf.pad(out, [[0, 0], [0, self.freq_bins - f_out], [0, 0], [0, 0]]),
                lambda: out
            )
        )
